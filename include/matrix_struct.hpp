...
/**
    ...
    @param row_index    uint32_t - Row Index
    @param col_index    uint32_t - Column Index
...
private:
    ...
    {
        if (
            (this->row_index != target.row_index) ||
            (this->col_index != target.col_index) ||
            (this->val != target.val))
        {
            return false;
        }
...
/**
    ...
    @param index uint32_t - index of row(for CSC), colum(for CSR) and value(for sparse vector).
...
/**
    ...
    @param row_with_val std::vector<IDX_VAL> - vector of IDX_VAL contains row index and value
*/
struct CSC
{
    MATRIX_INFO mat_data;
...
/**
    ...
    @param col_with_val std::vector<IDX_VAL> - vector of IDX_VAL contains column index and value
*/
struct CSR
{
public:
