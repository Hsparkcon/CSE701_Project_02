...
bool MATRIX_CONVERT::convert(const std::vector<uint32_t> &row_idx, const std::vector<uint32_t> &col_idx, const std::vector<double> &val, const uint32_t num_row, const uint32_t num_col, COO &coo)
{
    ...
    for (uint32_t ele_A = 0; ele_A < coo.mat_data.num_nz; ele_A++)
    {
        ...
        coo.mat_elements[ele_A].row_index = row_idx[ele_A];

        track_col = track_row < col_idx[ele_A] ? col_idx[ele_A] : track_row;
        coo.mat_elements[ele_A].col_index = col_idx[ele_A];
...
bool MATRIX_CONVERT::convert(const std::vector<uint32_t> &row_idx, const std::vector<uint32_t> &col_idx, const std::vector<double> &val, const uint32_t num_row, const uint32_t num_col, CSC &csc)
{
    COO *temp = new COO();
...
bool MATRIX_CONVERT::convert(const std::vector<uint32_t> &row_idx, const std::vector<uint32_t> &col_idx, const std::vector<double> &val, const uint32_t num_row, const uint32_t num_col, CSR &csr)
{
    COO *temp = new COO();
    this->convert(row_idx, col_idx, val, num_row, num_col, *temp);
...
bool MATRIX_CONVERT::convert(COO &coo, CSR &csr)
{
    ...
    {
        for (; ptr_tracker < coo.mat_elements[iter_nz].row_index; ptr_tracker++)
        {
            csr.row_ptr.push_back(iter_nz);
        }
        temp.index = coo.mat_elements[iter_nz].col_index;
...
bool MATRIX_CONVERT::convert(COO &coo, CSC &csc)
{
    ...
    for (uint32_t iter_nz = 0; iter_nz < coo.mat_data.num_nz; iter_nz++)
    {
        ...
        for (; ptr_tracker < coo.mat_elements[iter_nz].col_index; ptr_tracker++)
        {
            csc.col_ptr.push_back(iter_nz);
        }
        temp.index = coo.mat_elements[iter_nz].row_index;
        temp.val = coo.mat_elements[iter_nz].val;
...
bool MATRIX_CONVERT::convert(CSR &csr, COO &coo)
{
    ...
    for (uint32_t row_ptr = 0; row_ptr < csr.mat_data.num_row; row_ptr++)
    {
        ...
        {
            temp.row_index = row_ptr;
            temp.col_index = csr.col_and_val[row].index;
...
bool MATRIX_CONVERT::convert(CSR &csr, CSC &csc)
{
    COO *temp = new COO();
...
bool MATRIX_CONVERT::convert(CSC &csc, COO &coo)
{
    ...
    for (uint32_t col_ptr = 0; col_ptr < csc.mat_data.num_col; col_ptr++)
    {
        ...
        {
            temp.col_index = col_ptr;
            temp.row_index = csc.row_and_val[col].index;
...
bool MATRIX_CONVERT::convert(CSC &csc, CSR &csr)
{
    COO *temp = new COO();
...
bool MATRIX_CONVERT::convert(D_MATRIX &d_mat, COO &coo)
{
...
bool MATRIX_CONVERT::convert(COO &coo, D_MATRIX &d_mat)
{
    ...
    {
        d_mat.matrix[mat_ele.row_index][mat_ele.col_index] = mat_ele.val;
    }

    return true;
}

bool MATRIX_CONVERT::transpose(COO &input, COO &output)
{
    ...
    {
        output.mat_elements[ele_A].row_index = input.mat_elements[ele_A].col_index;
        output.mat_elements[ele_A].col_index = input.mat_elements[ele_A].row_index;
        output.mat_elements[ele_A].val = input.mat_elements[ele_A].val;
    }
    return true;
}

bool MATRIX_CONVERT::transpose(CSR &input, CSR &output)
{
    ...
    {
        for (; ptr_tracker < temp_coo.mat_elements[iter_nz].col_index; ptr_tracker++)
        {
            output.row_ptr.push_back(iter_nz);
        }
        temp.index = temp_coo.mat_elements[iter_nz].row_index;
...
bool MATRIX_CONVERT::transpose(CSC &input, CSC &output)
{
    MATRIX_INFO temp_info;
    temp_info.matrix_name = input.mat_data.matrix_name + "_Transposed";
    temp_info.num_row = input.mat_data.num_col;
    ...
    for (uint32_t iter_nz = 0; iter_nz < temp_coo.mat_data.num_nz; iter_nz++)
    {
        for (; ptr_tracker < temp_coo.mat_elements[iter_nz].row_index; ptr_tracker++)
        {
            output.col_ptr.push_back(iter_nz);
        }
        temp.index = temp_coo.mat_elements[iter_nz].col_index;
        temp.val = temp_coo.mat_elements[iter_nz].val;
...
void MATRIX_CONVERT::sort_COO(COO &s_mat, sort_target target)
{
    switch (target)
    {
