#include "../include/read_mtx.hpp"

READ_MTX::READ_MTX() {}

READ_MTX::~READ_MTX() {}

bool READ_MTX::load_matrix_mtx_ANSI_based(std::string file_name, COO &output_mat)
{
    FILE *load_mtx;
...
#define read_max 256
    ...
    {
        fscanf(load_mtx, "%u %u %lg\n", &temp.row_index, &temp.col_index, &temp.val);
        temp.row_index--;
        temp.col_index--;
...
bool READ_MTX::load_matrix_mtx_UNSAFE(std::string file_name, COO &output_mat)
{
    ...
    {
        // The following line used to skip the line only.
        // This continues until an encountering line not starting with '%'
    ...
    {
        load_mtx_file >> temp_data.row_index >> temp_data.col_index >> temp_data.val;
        if (index_check(num_row, num_col, temp_data.row_index, temp_data.col_index) == false)
        {
            return false;
        }
        else
...
bool READ_MTX::load_matrix_mtx_SAFE(std::string file_name, COO &output_mat)
{
    std::ifstream load_mtx_file;
    load_mtx_file.open(file_name, std::ios::in);
    if (load_mtx_file.is_open() == false)
    ...
    while (std::getline(load_mtx_file, read_line, '\n'))
    {
        ...
        {
            std::istringstream str_stream(read_line);
            str_stream >> temp.row_index >> temp.col_index >> temp.val;

            if (index_check(num_row, num_col, temp.row_index, temp.col_index) == false)
            {
                return false;
            }
            else
    ...
    // output_mat.mat_data = (MATRIX_INFO){file_name, num_col, num_row, num_nz};
    output_mat.mat_data.matrix_name = file_name;
    output_mat.mat_data.num_row = num_row;
    output_mat.mat_data.num_col = num_col;
    output_mat.mat_data.num_nz = num_nz;
...
bool READ_MTX::index_check(uint32_t row_limit, uint32_t col_limit, uint32_t row_ind, uint32_t col_ind) const
{
    if (row_ind > row_limit || col_ind > col_limit)
    {
        std::cout << "CORRUPTED DATA\n"
                  << "One of index value exceed row/column of matrix\n"
