#include "../include/read_svec.hpp"

READ_SVEC::READ_SVEC() {}

READ_SVEC::~READ_SVEC() {}

bool READ_SVEC::load_svec_ANSI_BASE(std::string file_name, S_VECTOR &output_vec)
{
    FILE *load_svec;
...
#define read_max 256
    ...
    {
        fscanf(load_svec, "%u %lg\n", &output_vec.vec_element[i].index, &output_vec.vec_element[i].val);
        output_vec.vec_element[i].index--;
    }
    fclose(load_svec);
...
bool READ_SVEC::load_svec_UNSAFE(std::string file_name, S_VECTOR &output_vec)
{
    ...
    {
        load_d_vec >> temp.index >> temp.val;
        temp.index--;
        output_vec.vec_element.push_back(temp);
    }
    load_d_vec.close();

    // output_vec.vec_data = (VECTOR_INFO){file_name, len_vector, num_nz};
    output_vec.vec_data.vec_name = file_name;
    output_vec.vec_data.len_vec = vec_len;
    output_vec.vec_data.num_nz = num_nz;
...
bool READ_SVEC::load_svec_SAFE(std::string file_name, S_VECTOR &output_vec)
{
    std::ifstream load_d_vec;
    load_d_vec.open(file_name, std::ios::in);
    if (load_d_vec.is_open() == false)
    ...
    while (std::getline(load_d_vec, read_line, '\n'))
    {
        ...
        {
            std::istringstream str_stream(read_line);
            str_stream >> temp.index >> temp.val;

            if (index_check(vec_len, temp.index) == false)
            {
                return false;
            }
            else
    ...
    // output_vec.vec_data = ((VECTOR_INFO){file_name, vec_len, num_nz});
    output_vec.vec_data.vec_name = file_name;
    output_vec.vec_data.len_vec = vec_len;
    output_vec.vec_data.num_nz = num_nz;
...
bool READ_SVEC::index_check(uint32_t idx_limit, uint32_t idx) const
{
    if (idx > idx_limit)
    {
        std::cout << "CORRUPTED DATA\n"
                  << "One of index value exceed length of vector\n"
