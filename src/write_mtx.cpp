#include "../include/write_mtx.hpp"

WRITE_MTX::WRITE_MTX() {}

WRITE_MTX::~WRITE_MTX() {}

bool WRITE_MTX::write_mtx_ANSI_based(std::string file_name, COO &output_mat)
{
    FILE *write_mtx;
    ...
    for (uint32_t ele_A = 0; ele_A < output_mat.mat_data.num_nz; ele_A++)
    {
        fprintf(write_mtx, "%u %u %lg\n",
                output_mat.mat_elements[ele_A].row_index + 1,
                output_mat.mat_elements[ele_A].col_index + 1,
...
bool WRITE_MTX::write_mtx_UNSAFE(std::string file_name, COO &output_mat)
{
    ...
    {
        output_line =
            std::to_string(output_mat.mat_elements[ele_A].row_index + 1) + " " +
            std::to_string(output_mat.mat_elements[ele_A].col_index + 1) + " " +
...
bool WRITE_MTX::write_mtx_SAFE(std::string file_name, COO &output_mat)
{
    std::ofstream write_mtx_file;
    write_mtx_file.open(file_name, std::ios::out);
    if (write_mtx_file.is_open() == false)
