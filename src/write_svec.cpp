#include "../include/write_svec.hpp"

WRITE_SVEC::WRITE_SVEC() {}

WRITE_SVEC::~WRITE_SVEC() {}

bool WRITE_SVEC::write_svec_ANSI_based(std::string file_name, S_VECTOR& output_vec)
{
    FILE* write_svec;
    ...
    for (uint32_t ele_A = 0; ele_A < output_vec.vec_data.num_nz; ele_A++)
    {
        fprintf(write_svec, "%u %lg\n",
            output_vec.vec_element[ele_A].index + 1,
...
bool WRITE_SVEC::write_svec_UNSAFE(std::string file_name, S_VECTOR& output_vec)
{
    ...
    {
        output_line =
            std::to_string(output_vec.vec_element[ele_A].index + 1) + " " +
...
bool WRITE_SVEC::write_svec_SAFE(std::string file_name, S_VECTOR& output_vec)
{
    std::ofstream write_svec_file;
    write_svec_file.open(file_name, std::ios::out);
    if (write_svec_file.is_open() == false)
